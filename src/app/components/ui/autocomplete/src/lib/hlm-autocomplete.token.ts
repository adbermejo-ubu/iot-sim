...
const HlmAutocompleteConfigToken = new InjectionToken<
    HlmAutocompleteConfig<unknown, unknown>
>("HlmAutocompleteConfig");

export function provideHlmAutocompleteConfig<T, V = T>(
    config: Partial<HlmAutocompleteConfig<T, V>>,
): ValueProvider {
    return {
...
export function injectHlmAutocompleteConfig<T, V = T>(): HlmAutocompleteConfig<
    T,
    V
> {
    return (
        (inject(HlmAutocompleteConfigToken, {
            optional: true,
        }) as HlmAutocompleteConfig<T, V> | null) ?? getDefaultConfig()
    );
}
